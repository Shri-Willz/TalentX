...
export default function ResumeMaker() {
  ...
  return (
    <>
      <Navbar />
      <main className="min-h-screen w-full bg-background text-gray-100">
        <div className="w-full px-4 md:px-10 py-12 flex flex-col lg:flex-row gap-10 lg:gap-12 max-w-[1600px] mx-auto">
          <section className="flex-1">
            ...
            <div className="flex flex-col gap-4">
              {sections.map(({ title, status, icon: Icon }, i) => (
                <div
                  key={i}
                  className="flex items-center justify-between bg-black/40 border border-white/10 rounded-2xl p-5 hover:border-accent hover:bg-black/60 backdrop-blur-md transition-all duration-300"
                >
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <p
                        className={`text-sm ${status === "Completed" ? "text-green-400" : "text-gray-400"
                          }`}
                      >
                        {status}
                      </p>
          ...
          <aside className="lg:w-[40%] bg-black/40 border border-white/10 rounded-2xl p-6 backdrop-blur-md hover:border-blue-500 transition-all duration-300 flex flex-col">
            ...
            <div className="flex-1 flex items-center justify-center border border-white/10 rounded-xl bg-black/30 backdrop-blur-sm transition-all duration-300">
